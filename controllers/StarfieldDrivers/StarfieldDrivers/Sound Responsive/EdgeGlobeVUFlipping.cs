...
namespace StarfieldDrivers.Drivers
{
    ...
    class EdgeGlobeVUFlipping : IStarfieldDriver
    {
        ...
        List<bool> globeEnable = new List<bool>();
        ...
        void soundProcessor_OnArtifactDetected(Artifact artifact)
        {
            if (!transitioning)
            {
                ...
                {
                    val = rand.Next(3);
                    for (int i = 0; i < globeEnable.Count; i++)
                    {
                        globeEnable[i] = false;
                    }
                    switch (val)
            ...
            for (ulong x = 0; x < Starfield.NumX; x++)
            {
                for (ulong y = 0; y < Starfield.NumY; y++)
                {
                    for (ulong z = 0; z < Starfield.NumZ; z++)
                    {
                        ...
                        {
                            if (globeEnable[globes.IndexOf(globe)])
                            {
