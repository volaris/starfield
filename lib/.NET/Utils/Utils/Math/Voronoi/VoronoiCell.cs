...
namespace VoronoiNS
{
    public class VoronoiCell
    {
                ...
                if (nextEdge == null)
                {
                  ...
                  */
                    ...
                    if (nextEdge == null)
                    {
                        ...
                        Debugger.Log(0, "[ERROR]", "Major error: was trying to close the loop of an outer Cell, but couldn't find a restart point");
                        break;
                    }
                }
        ...
        private List<Vec2D> _cachedGizmoOutline;

        void OnDrawGizmosSelected()
        {
            Debugger.Log(0, "[INFO]", "GIZMOS SELECTED");
            // if (_cachedGizmoOutline == null)
            {
                _cachedGizmoOutline = new List<Vec2D>();
            ...
            Debugger.Log(0, "[INFO]", "GIZMOS SELECTED: NODES = " + _cachedGizmoOutline.Count);

            Gizmos.color = new Color(Random.Range(0, 1f), Random.Range(0, 1f), Random.Range(0, 1f));

            for (int i = 0; i + 1 < _cachedGizmoOutline.Count; i++)
            {
                Vec2D left = _cachedGizmoOutline[i];
                Vec2D right = _cachedGizmoOutline[i + 1];
