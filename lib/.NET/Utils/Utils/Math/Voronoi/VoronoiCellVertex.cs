...
namespace VoronoiNS
{
    public class VoronoiCellVertex
    {
        ...
        public Vec2D positionInDiagram;
        public string name;
 ...
 Note: Unity has many serialization bugs with arrays, its just not worth the
        ...
        */
        ...
        public void AddCellWithSideEffects(VoronoiCell c)
        {
            ...
            {
                Debugger.Log(0, "[ERROR]", "Major error: you re-added a cell (" + c.name + ") to a vertex  (" + this.name + ")  that already had that cell; this suggests you've run a cel/vertex generation algorithm twice on the same VoronoiDiagram object (should never happen!)");
            }
            ...
            {
                Debugger.Log(0, "[ERROR]", "Cannot add a fourth cell  (" + c.name + ")  to a vertex  (" + this.name + ") !");
                return;
            }

            Debugger.Log(0, "[INFO]", "  vertex  (" + this.name + ") + cell = (" + (cell1 == null ? "" : cell1.name) + ", " + (cell2 == null ? "" : cell2.name) + ", " + (cell3 == null ? "" : cell3.name) + ")  to ");
        }
        ...
        */
        ...
        public VoronoiCellEdge GetOtherEdgeContainingCell(VoronoiCellEdge currentEdge, VoronoiCell targetCell)
        {
            ...
            if (edge3 != null)
            {
                ...
                Debugger.Log(0, "[ERROR]", "Cannot find another edge from this vertex (" + this + ") that borders cell: " + targetCell);
            }
            return null;
        }
    }
