...
namespace Delaunay
{
	...
	public sealed class Edge
	{
		...
		{
            return (float)leftSite.Coord.DistanceTo(rightSite.Coord);
		}
        ...
        private Dictionary<Side, Vec2D> _clippedVertices;
        public Dictionary<Side, Vec2D> clippedEnds
        {
...
//			}
		...
		public void ClipVertices (Rect bounds)
		{
            ...
            _clippedVertices = new Dictionary<Side, Vec2D>();
			if (vertex0 == _leftVertex) {
                _clippedVertices[Side.LEFT] = new Vec2D(x0, y0);
                _clippedVertices[Side.RIGHT] = new Vec2D(x1, y1);
			} else {
                _clippedVertices[Side.RIGHT] = new Vec2D(x0, y0);
                _clippedVertices[Side.LEFT] = new Vec2D(x1, y1);
			}
		}
