...
namespace Delaunay
{
	...
	public sealed class SiteList: IDisposable
	{
		...
		{
			if (_sorted == false) {
				Debugger.Log(0, "[ERROR]", "SiteList::next():  sites have not been sorted");
			}
		...
		public List<List<Vec2D>> Regions (Rect plotBounds)
		{
			List<List<Vec2D>> regions = new List<List<Vec2D>> ();
			Site site;
			for (int i = 0; i< _sites.Count; i++) {
				site = _sites [i];
		 ...
		 * 
		 */
		public Vec2D NearestSitePoint (/*proximityMap:BitmapData,*/float x, float y)
		{
