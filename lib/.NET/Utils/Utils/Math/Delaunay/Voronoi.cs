...
namespace StarfieldUtils.MathUtils
{
	public sealed class Voronoi: IDisposable
	{
		...
		private Dictionary <Vec2D,Site> _sitesIndexedByLocation;
...
//			_plotBounds = null;
		...
		public Voronoi (List<Vec2D> points, List<uint> colors, Rect plotBounds)
		{
			_sites = new SiteList ();
			_sitesIndexedByLocation = new Dictionary <Vec2D,Site> (); // XXX: Used to be Dictionary(true) -- weak refs. 
		...
		private void AddSites (List<Vec2D> points, List<uint> colors)
		{
		...
		private void AddSite (Vec2D p, uint color, int index)
		{
			...
			float weight = (float)rand.NextDouble() * 100f;
		...
		public List<Vec2D> Region (Vec2D p)
		{
		...
		public List<Vec2D> NeighborSitesForSite (Vec2D coord)
		{
		...
		public List<LineSegment> VoronoiBoundaryForSite (Vec2D coord)
		{
			return DelaunayHelpers.VisibleLineSegments (DelaunayHelpers.SelectEdgesForSitePoint (coord, _edges));
		}

		public List<LineSegment> DelaunayLinesForSite (Vec2D coord)
		{
		...
		public List<Vec2D> HullPointsInOrder ()
		{
		 ...
		 * 
		 */
		public Vec2D NearestSitePoint (/*BitmapData proximityMap,*/float x, float y)
		{
			return _sites.NearestSitePoint (/*proximityMap,*/x, y);
		}
		
		public List<Vec2D> SiteCoords ()
		{
		...
		private void FortunesAlgorithm ()
		{
			...
			Vertex v, vertex;
			Vec2D newintstar = Vec2D.Zero; //Because the compiler doesn't know that it will have a value - Julian
			Side leftRight;
		...
		public static int CompareByYThenX (Site s1, Vec2D s2)
		{
			if (s1.y < s2.Y)
				return -1;
			if (s1.y > s2.Y)
